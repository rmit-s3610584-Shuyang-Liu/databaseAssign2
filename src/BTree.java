...
public class BTree implements Serializable {
	...
	// private static int M = (HeapPage.size - 100) / Integer.BYTES / 2; // max
	// children per B-tree node = M-1
	...
	private List<Integer> search(Node x, int key) {
		...
		for (int i = 1; i <= x.m; i++) {
			if (eq(key, data[i].key))
				return (List<Integer>) data[i].value;
			else if (less(key, data[i].key))
		...
		if (h.m > MAX_KEYS) { // need split
			...
			if (h.parent != null) {
				...
				for (j = 1; j <= h.parent.m; j++) {
					if (less(h.data[mid].key, h.parent.data[j].key))
						break;
				}
